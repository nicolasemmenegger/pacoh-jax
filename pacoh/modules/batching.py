...
def get_batched_module(
    transformed: Union[Transformed, MultiTransformed, TransformedWithState, MultiTransformedWithState],
    ...
    with_state = isinstance(transformed, TransformedWithState) or isinstance(
        transformed, MultiTransformedWithState
    )
    ...
    if not multi:
        ...
        for (
            fname,
            func,
        ) in apply_fns._asdict().items():  # TODO is there a public interface to this
            apply_dict[fname] = jit(vmap(func, in_axes=base_in_axes + data_in_axes[fname]))
            apply_dict_batched_inputs[fname] = jit(
                vmap(func, in_axes=base_in_axes + data_in_axes_batched[fname])
            )

        return (
            batched_init,
            apply_fns.__class__(**apply_dict),
            apply_fns.__class__(**apply_dict_batched_inputs),
        )
...
transform_and_batch_module_with_state = functools.partial(
    _transform_batch_base, purify_fn=hk.transform_with_state
)
multi_transform_and_batch_module = functools.partial(_transform_batch_base, purify_fn=hk.multi_transform)
multi_transform_and_batch_module_with_state = functools.partial(
    _transform_batch_base, purify_fn=hk.multi_transform_with_state
)
...
if __name__ == "__main__":
    ...
    (
        init_batched,
        batched_forward,
        batched_forward_batch_inputs,
    ) = get_pure_batched_nn_functions(1, (32, 32), jax.nn.elu)
    print(init_batched)
    ...
    print(
        "It's very nice, because I get the output of 3 gps here: POSTERIOR\n",
        output.loc,
    )
