...
class BraninEnvironment(BenchmarkEnvironment):
    domain = ContinuousDomain(np.array([-5.0, 0.0]), np.array([10.0, 15.0]))
    ...
    def _construct_f_from_params(self, params):
        def fun(x):
            ...
            if x.ndim == 2:
                ...
                x1, x2 = x[0], x[1]
            return (
                params["a"] * (x2 - params["b"] * x1**2 + params["c"] * x1 - params["r"]) ** 2
                + params["s"] * (1 - params["t"]) * np.cos(x1)
                + params["s"]
            )
...
class MixtureEnvironment(BenchmarkEnvironment):
    domain = ContinuousDomain(np.array([-10.0]), np.array([10.0]))
    has_constraint = True
    ...
    def _construct_f_from_params(self, params):
        def fun(x):
            ...
            gaussian = (
                1
                / np.sqrt(2 * np.pi)
                * np.exp(-0.5 * (np.linalg.norm(x - params["loc2"], axis=-1) / (d * 2)) ** 2)
            )
            cauchy3 = 1 / (np.pi * (1 + (np.linalg.norm(x - params["loc3"], axis=-1) / (d * 4)) ** 2))
            return (
                -params["scales"][0] * 2.0 * cauchy1
                - 1.5 * params["scales"][1] * gaussian
                - params["scales"][2] * 1.8 * cauchy3
                - 1
            )
    ...
    def _construct_q_from_params(self, params):
        def constr_fun(x):
            ...
            # cauchy3 = 1 / (np.pi * (1 + (np.linalg.norm(x - params['loc3'], axis=-1) / (d * 4)) ** 2))
            return -params["scales"][0] * 2.0 * cauchy1 - 2 * params["scales"][1] * gaussian + 0.5

        self.q_constraint = constr_fun


class ArgusSimEnvironment(Environment):
    domain = ContinuousDomain(np.array([50.0, 300.0, 500.0]), np.array([400.0, 1200.0, 4000.0]))
    has_constraint = True
    default_params = {
        "Ts": 5e-5,  # Sampling time simulation in s
    ...
    def _setup_matlab_enginge(self):
        ...
        os.chdir(
            os.path.join(BASE_DIR, "argus_sim")
        )  # TODO: This might be dangerous, any better idea how to solve this?
    ...
    def normalization_stats(self):
        ...
        stats = {
            ...
            "y_mean": np.array((y_max + y_min) / 2.0),
