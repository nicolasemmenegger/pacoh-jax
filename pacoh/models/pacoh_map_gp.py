...
from pacoh.util.initialization import initialize_model_with_state
...
class PACOH_MAP_GP(RegressionModelMetaLearned):
    def __init__(self,
        ...
        # TODO try if it makes difference
        ...
        def target_post_prob(particle, meta_xs_batch, meta_ys_batch):
            ...
            return total_mll # - 0.002*jnp.sum(reg)
        ...
        self.optimizer_state = self.optimizer.init(self.particle)

    def _meta_step(self, mini_batch) -> float:
        # I'd like a minibatch to be
        xs_batch, ys_batch = mini_batch
        # print(self.target(self.particle, xs_batch, ys_batch))
...
if __name__ == "__main__":
    ...
    for weight_decay in [0.5]:
        ...
        for i in range(40):
            n_iter = 500
            pacoh_map.meta_fit(meta_train_data, log_period=1000, num_iter_fit=n_iter)
            ...
            # pred_prior = pacoh_map.meta_predict(jnp.zeros((0,1)), jnp.zeros((0,)), x_plot).mean
