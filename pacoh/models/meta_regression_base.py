...
class RegressionModelMetaLearned(RegressionModel, metaclass=AbstractAttributesABCMeta):
    ...
    def __init__(
        ...
        num_tasks: int = None,
        flatten_ys=False,
        num_iter_meta_fit: int = -1,
        minibatch_at_dataset_level: bool = False,
        dataset_batch_size: Optional[int] = None,
    ):
        super().__init__(input_dim, output_dim, normalize_data, normalizer, flatten_ys, random_state)
        if num_tasks is None:
            raise ValueError(
                "Please specify the number of tasks you intend to train on at initialisation time"
            )
    ...
    def meta_fit(
        ...
        if self._provided_normaliser is None:
            self._normalizer = DataNormalizer.from_meta_tuples(
                meta_train_tuples, True, flatten_ys=self.flatten_ys
            )
        ...
        for i, batch in zip(p_bar, dataloader):
            ...
            avg_loss = (
                itr - 1
            ) / itr * avg_loss + 1 / itr * instantaneous_loss  # running average of loss over log_period
    ...
    def eval_datasets(self, test_tuples, **kwargs):
        ...
        results_tup = [
            (res["avg. ll"], res["rmse"], res["calib err."], res["calib err. chi2"]) for res in results
        ]
    ...
    def _get_meta_dataloader(
        ...
        return_array=False,
    ) -> Union[MetaDataLoaderTwoLevel, MetaDataLoaderOneLevel]:
        """
        ...
        if return_array:
            if dataset_batch_size == -1 or dataset_batch_size is None:
                ...
                assert np.all(lengths == lengths[0]), (
                    "If minibatching at task level without specifying a batch_size,"
